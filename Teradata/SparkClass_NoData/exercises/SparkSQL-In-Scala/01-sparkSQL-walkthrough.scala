...
val s3flights = spark.read.parquet("hdfs:///data/flightdata/parquet-trimmed")
...
val delays = flights.groupBy("carrier", "dest", "origin").mean("arrdelayminutes")
delays.printSchema
...
spark.time(flights.groupBy("carrier", "dest", "origin").
  mean("arrdelayminutes").
  sort(desc("avg(arrdelayminutes)")).show())

// Here's what I got from the first run on EMR.
// Time taken: 18743 ms

// Now persist the flights dataset by typing `flights.persist()`. 
// See how long that command take now.
...
// See how long these operations take now
// Time taken: 16630 ms

// Persistance doesn't seem to help much, does it?

// Actually it helps more than you think. Remember that Scala uses lazy
// evaluation for its results. When we told Spark to persist the
// `flights` DataFrame, it evaluated that lazily; it has no idea whether
// that DataFrame has been computed yet or not -- it just remembered that
// it should cache it when it next computes it. Therefore, the second run
// of the command was the first one that cached `flights`.

// If this is the case, we should see substantially better performance if
// we run the command one more time
...
// The third time we ran that command, it ran in roughly a fifth of the time taken for the first run.
